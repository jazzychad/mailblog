...
module Jekyll
  ...
  class SitemapGenerator < Generator
    ...
    def generate_content(site)
      ...
      site.pages.each{ |page|
        ...
        begin
          ...
          mod_date = Time.now # done for dynamically generated pages (e.g. from another plugin)
        end
      ...
      for post in posts do
        result += entry(post.generated_path, post.date)
      end

      # Next, find all the bits.
      posts = site.site_payload['site']['bits']
      for post in posts do
        result += entry(post.generated_path, post.date)
      end
    ...
    def entry(path, date)
        ...
        path = path.gsub(/\.(md|markdown|textile)$/i, '.html')
      "
  <url>
